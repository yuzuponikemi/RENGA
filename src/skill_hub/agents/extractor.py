...
- triggers: このスキルを呼び出す典型的な自然言語フレーズ（3〜5個）。ユーザーが実際に入力しそうな短い言葉。例: ["コードをレビューして", "PRを確認して", "コードチェック"]
...
class ExtractorAgent:
    def __init__(self, client: OpenAI, model: str, registry: ContributorRegistry | None = None):
        self.client = client
        self.model = model
    ...
    def _extract_skill(self, task_type: str, logs: list[CopilotLog]) -> ExtractedSkill:
        ...
        handles = [self.registry.get_handle(uid) for uid in user_ids] if self.registry else []
